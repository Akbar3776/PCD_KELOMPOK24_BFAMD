...
def check_url(url):
    ...
    try:
        headers = {
            "Range": "bytes=0-10",
            "User-Agent": USER_AGENT,  # Updated from "MyTestAgent"
            "Accept": "*/*"
        }
...
def main():
    ...
    with open("BAFMD_unavailable_urls.tsv","w") as f:
        for image_url_pair in tqdm(image_urls):
            ...
            # --- THE FIX: Create a request object with a User-Agent ---
            req = urllib2.Request(
                url, 
            ...
            if filename in train_set_image_names:
                ...
                with open(os.path.join(train_dir,filename+"."+url.split(".")[-1]),"wb") as image_f:
                    # Use req instead of url
                    image_f.write(urllib2.urlopen(req).read())
